...
func CallGoFunction(target string, name string, params ...interface{}) (interface{}, error) {
	...
	if action.Results[1] != nil {
		Utility.Log(Utility.FunctionName(), Utility.FileLine(), "action  ", name, action.Results[0])
		Utility.Log(Utility.FunctionName(), Utility.FileLine(), "action error ", name, action.Results[1])
