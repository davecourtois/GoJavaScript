...
import (
	...
	"github.com/davecourtois/GoJavaScript"
...
func NewClient(address string, port int, name string) (*Client, error) {
	...
	if err != nil {
		Utility.Log(Utility.FunctionName(), Utility.FileLine(), "Fail to connect with server ", address, ":", port, err)
		return nil, err
	}
...
func (self *Client) startServerProcess(name string, address string, port int) error {
	...
	if err != nil {
		...
		if err == nil {
			...
			err = self.srv.Start()
			if err != nil {
				Utility.Log(Utility.FunctionName(), Utility.FileLine(), "Fail to start GoJavaScriptServer", err)
				return nil
			}
		}
		return err
	...
	if port == 0 {
		...
		for scanner.Scan() {
			...
			for i := 0; i < 500; i++ {
				...
				if err == nil {
					Utility.Log(Utility.FunctionName(), Utility.FileLine(), "Peer connected at ", address, ":", port)
					return nil
				}
			}
		}

		// The function wait for scanner at line 103 to get the port number.
		self.srv.Wait()
			...
			go func(a *GoJavaScript.Action) {
				...
				if len(a.Target) > 0 {
					if a.Target == "Client" {
						...
						if target == nil {
							Utility.Log(Utility.FunctionName(), Utility.FileLine(), "fail to retreive target ", a.Target)
						}
					}
				}
				...
				if isJsObject {
					...
					if err != nil {
						Utility.Log(Utility.FunctionName(), Utility.FileLine(), "call Method: ", a.Name)
						for i := 0; i < len(params); i++ {
							Utility.Log("param ", i, params[i], reflect.TypeOf(params[i]).String())
						}
						str, _ := Utility.ToJson(target)
						Utility.Log(Utility.FunctionName(), Utility.FileLine(), "target: ", str)
						Utility.Log(Utility.FunctionName(), Utility.FileLine(), "error: ", err)
					}
				}
