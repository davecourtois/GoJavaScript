		...
		for self.isRunning {
			...
			if decoder != nil {
				...
				if err == nil {
					...
					if err.Error() == "EOF" {
						Utility.Log(Utility.FunctionName(), Utility.FileLine(), "Connection close normaly for peer ", self.address, self.port)
						os.Exit(0) // exit the process normaly...
					}
					return
				}
			} else {
				Utility.Log("Peer.go line 107 fail to create decoder tcp connection!")
				return
			}
		}
	}()
	...
	for self.isRunning {
		...
		case msg := <-self.send_chan:
			go func(self *Peer, msg *Message) {
				...
				if err != nil {
					Utility.Log(Utility.FunctionName(), Utility.FileLine(), "marshaling error: ", err, os.Args)
					os.Exit(0)
				}
			}(self, msg)
		case <-self.stop_chan:
			Utility.Log(Utility.FunctionName(), Utility.FileLine(), "close peer ", self.address, self.port)
			self.conn.Close()
...
func (self *Peer) Listen() {
	...
	Utility.Log(Utility.FunctionName(), Utility.FileLine(), "connection open from server at port ---> ", self.port)
...
func (self *Peer) Connect(address string, port int) error {
	...
	if err != nil {
		Utility.Log(Utility.FunctionName(), Utility.FileLine(), "connection error: ", err)
		return err
	}
